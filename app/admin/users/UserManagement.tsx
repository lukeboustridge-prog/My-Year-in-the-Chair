...
export default function UserManagement({ initialUsers }: Props) {
  ...
  const updateApproval = async (userId: string, approved: boolean) => {
    ...
    try {
      ...
      console.error("APPROVAL_UPDATE", err);
      setError("Unable to update approval status. Please try again.");
    } finally {
      setBusyId(null);
    }
  };
              ...
              {users.map((user) => {
                ...
                return (
                  <tr key={user.id} className="border-t">
                    ...
                    <td className="py-2 pr-3 uppercase text-slate-500">{user.role}</td>
